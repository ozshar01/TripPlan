...
window.SheetsSync = (() => {
  ...
  function cellValue(cell) {
    ...
    if (v instanceof Date) {
      ...
      const datePart = `${v.getFullYear()}-${pad2(v.getMonth() + 1)}-${pad2(v.getDate())}`;
      return hasTime ? `${datePart} ${pad2(v.getHours())}:${pad2(v.getMinutes())}` : datePart;
    }
    if (Array.isArray(v)) {
      // timeofday cell: [hours, minutes, seconds, millis]
  ...
  function gvizToRows(json) {
    ...
    return (table.rows || [])
      .map((r) => (r.c || []).map((cell) => cellValue(cell)))
