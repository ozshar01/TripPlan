(() => {
  ...
  // Normalizes a date value to 'YYYY-MM-DD'. Accepts values already in that
  // format, and common non-ISO text like "20/9/2026" or "20.9.2026" (which
  // people naturally type into a spreadsheet even when asked for ISO).
  // Returns '' for empty/unrecognized input rather than throwing.
  const toISODate = (d) => {
    if (!d) return '';
    const s = String(d)
      .replace(/[\u200E\u200F\u202A-\u202E\u2066-\u2069]/g, '')
  ...
  const fmt = (d) => {
    ...
    if (isNaN(dt)) return String(d); // unrecognized format: show the raw text instead of "Invalid Date"
  ...
  const currentDay = () => {
    ...
    return D.days.find((d) => toISODate(d.date) === iso) || D.days[0];
  };
  ...
  // --- Dark Mode ---
  // Default follows the system's color scheme; a manual choice from the
  // button below overrides that and is remembered across visits.
  const DARK_PREF_KEY = 'japanTripDarkPrefV1';
  function applyDarkMode() {
    const saved = localStorage.getItem(DARK_PREF_KEY); // 'dark' | 'light' | null
    const dark =
      saved === 'dark'
        ? true
        : saved === 'light'
    ...
    document.body.classList.toggle('auto-dark', dark);
  }
  applyDarkMode();
  if (window.matchMedia) {
    const mq = window.matchMedia('(prefers-color-scheme: dark)');
    const onSystemChange = () => {
      if (!localStorage.getItem(DARK_PREF_KEY)) applyDarkMode();
    };
    if (mq.addEventListener) mq.addEventListener('change', onSystemChange);
    else if (mq.addListener) mq.addListener(onSystemChange);
  }
  document.querySelector('#darkBtn').onclick = () => {
    const isDarkNow = document.body.classList.contains('auto-dark');
    localStorage.setItem(DARK_PREF_KEY, isDarkNow ? 'light' : 'dark');
    applyDarkMode();
  };
